...
TEST_CASE("x2apic_ctl_check_gpa_op")
{
    x2apic_ctl ctrl;
    ...
    CHECK(ctrl.check_gpa_op(0xFEE00020ULL, lapic_ctl::write) == -1);     // ID Write
    CHECK(ctrl.check_gpa_op(0xFEE00090ULL, lapic_ctl::read) == -1);      // APR Read
    CHECK(ctrl.check_gpa_op(0xFEE00090ULL, lapic_ctl::write) == -1);     // APR Write
    CHECK(ctrl.check_gpa_op(0xFEE000C0ULL, lapic_ctl::read) == -1);      // RRD Read
    CHECK(ctrl.check_gpa_op(0xFEE000C0ULL, lapic_ctl::write) == -1);     // RRD Write
    CHECK(ctrl.check_gpa_op(0xFEE000D0ULL, lapic_ctl::write) == -1);     // LDR Write
    CHECK(ctrl.check_gpa_op(0xFEE000E0ULL, lapic_ctl::read) == -1);      // DFR Read
    CHECK(ctrl.check_gpa_op(0xFEE000E0ULL, lapic_ctl::write) == -1);     // DFR Write
    CHECK(ctrl.check_gpa_op(0xFEE00280ULL, lapic_ctl::write) == 0x28U);  // ESR Write
    CHECK(ctrl.check_gpa_op(0xFEE00310ULL, lapic_ctl::read) == -1);      // ICR High Read
    CHECK(ctrl.check_gpa_op(0xFEE00310ULL, lapic_ctl::write) == -1);     // ICR High Write
    CHECK(ctrl.check_gpa_op(0xFEE003F0ULL, lapic_ctl::read) == -1);      // Self IPI Read
    CHECK(ctrl.check_gpa_op(0xFEE003F0ULL, lapic_ctl::write) == 0x3FU);  // Self IPI Write
}

TEST_CASE("x2apic_ctl_check_msr_op")
{
    x2apic_ctl ctrl;
    ...
    // x2apic vs xapic register conflicts
    CHECK(ctrl.check_msr_op(0x00000802ULL, lapic_ctl::write) == -1);     // ID Write
    CHECK(ctrl.check_msr_op(0x00000809ULL, lapic_ctl::read) == -1);      // APR Read
    CHECK(ctrl.check_msr_op(0x00000809ULL, lapic_ctl::write) == -1);     // APR Write
    CHECK(ctrl.check_msr_op(0x0000080CULL, lapic_ctl::read) == -1);      // RRD Read
    CHECK(ctrl.check_msr_op(0x0000080CULL, lapic_ctl::write) == -1);     // RRD Write
    CHECK(ctrl.check_msr_op(0x0000080DULL, lapic_ctl::write) == -1);     // LDR Write
    CHECK(ctrl.check_msr_op(0x0000080EULL, lapic_ctl::read) == -1);      // DFR Read
    CHECK(ctrl.check_msr_op(0x0000080EULL, lapic_ctl::write) == -1);     // DFR Write
    CHECK(ctrl.check_msr_op(0x00000828ULL, lapic_ctl::write) == 0x28U);  // ESR Write
    CHECK(ctrl.check_msr_op(0x00000831ULL, lapic_ctl::read) == -1);      // ICR High Read
    CHECK(ctrl.check_msr_op(0x00000831ULL, lapic_ctl::write) == -1);     // ICR High Write
    CHECK(ctrl.check_msr_op(0x0000083FULL, lapic_ctl::read) == -1);      // Self IPI Read
    CHECK(ctrl.check_msr_op(0x0000083FULL, lapic_ctl::write) == 0x3FU);  // Self IPI Write
}

TEST_CASE("x2apic_ctl_read_register")
{
    x2apic_ctl ctrl;
...
TEST_CASE("x2apic_ctl_write_register")
{
    x2apic_ctl ctrl;
